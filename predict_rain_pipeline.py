...
def load_and_preprocess_data(filepath="data/weatherAUS.csv"):
    ...
    return X_train_scaled, X_test_scaled, y_train, y_test, scaler, list(X_train.columns), encoders


#model training
def train_and_save_models():
    
    X_train, X_test, y_train, y_test, scaler, feature_names, encoders = load_and_preprocess_data()
    ...
    models = {
        ...
        "rainfall_random_forest_classification": RandomForestClassifier(n_estimators=100,max_depth=10,random_state=42),
    ...
    results_df.to_csv("assignment2.csv", index=False)
    print("Training completed! Metrics saved to assignment2.csv")
    print(results_df)
